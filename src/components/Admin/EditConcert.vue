...
export default {
  ...
  created() {
    this.place = '' + this.concert.nbrPlace
    this.description = '' + this.concert.description
    this.lieu = '' + this.concert.lieu
    this.imageUrl = '' + this.concert.img
    this.date = this.concert.date
    this.time = this.concert.heureDeb
    this.date = this.concert.date
  },
  methods: {
    editConcert() {
      API.editConcert(this.concert.id, this.lieu, this.place, this.description, this.date, this.imageUrl, this.time).then((result) => {
        console.log(result)
        this.updateConcert()
        this.dialog = false
      })
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="12" md="6">
                <v-text-field label="Nombre de place*" required v-model="place"></v-text-field>
              </v-col>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="editConcert(concert)">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
